...
artqrcode = function artqrcode(qrinfo, imginfo) {
	return new Promise((resolve, reject) => {
		...
		var cyt = document.getElementById(qrinfo.canvasid).getContext("2d");
...
function pain(cyt, arrayName, width, height, imgurl) {
...
function painEye(cyt, eyeImguUrl, datalistlength, unit) {
	...
	//浏览器加载图片完毕后再绘制图片
	img.onload = () => {
		cyt.drawImage(img, 0, 0, 7 * unit, 7 * unit);
		cyt.drawImage(img, (datalistlength - 7) * unit, 0 * unit, 7 * unit, 7 * unit);
		cyt.drawImage(img, 0 * unit, (datalistlength - 7) * unit, 7 * unit, 7 * unit);
	}
}
/*
